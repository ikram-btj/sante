...
<script type="text/javascript" src="css/formoid_files/formoid1/jquery.min.js"></script>
  ...
  <center>
<h2>Liste de vaccinations</h2>
          
  <table class="table table-hover"  style ="margin-left:200px;width:850px;">
    <thead>
      <tr>
        <th><b>Type vaccin:</b></th>
		<th><b> Nombre d'injection:</b></th>
        <th>Numéro d'injection:</th>
        <th>Date injection:</th>
		<th>Date de prochain injection:</th>
...
//while ($champ = mysql_fetch_object($ligne) )
 ...
 $bd = mysql_query("SELECT * FROM injection where id_p = '$id_p' ") ;
while($base = mysql_fetch_object($bd)){
 echo "
           ...
           <tr>
        <td>$base->nom_vaccin</td>
		<td>$base->nbr_injection</td>
        <td>$base->num_injection</td>
		<td>$base->date_inj</td>
        <td>$base->date_rappel</td>
